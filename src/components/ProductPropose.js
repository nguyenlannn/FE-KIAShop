...
const ProductPropose = (props) => {
    ...
    useEffect(() => {
        fetch(BasicApi.searchProductByProductId(`sort=id:desc&size=6&page=${page}`,props.productId.id).url)
            .then((res) => res.json())
            .then((o) => setProduct(o));
    }, [page]);
    ...
    return <>
        <h3 className="text-warning" style={{marginTop: 20}}>Sản phẩm đề xuất</h3>
        <hr/>
        <div className="row">
